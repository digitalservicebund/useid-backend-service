...
Go
to [https://sonarcloud.io](https://sonarcloud.io/dashboard?id=digitalservice4germany_kotlin-application-template)
for the results.
...
```bash
docker run -p8080:8080 "ghcr.io/digitalservice4germany/kotlin-application-template:$(git log -1 origin/main --format='%H')"
```
...
docker run -p8080:8080 ghcr.io/digitalservice4germany/kotlin-application-template
```
...
```bash
COSIGN_EXPERIMENTAL=1 cosign verify "ghcr.io/digitalservice4germany/kotlin-application-template:$(git log -1 origin/main --format='%H')"
```
...
export CONTAINER_IMAGE_NAME=digitalservice4germany/kotlin-application-template
...
docker push "ghcr.io/digitalservice4germany/kotlin-application-template:$(git log -1 --format='%H')"
```
...
```bash
./gradlew bootBuildImage
trivy image --severity HIGH,CRITICAL ghcr.io/digitalservice4germany/kotlin-application-template:latest
```
